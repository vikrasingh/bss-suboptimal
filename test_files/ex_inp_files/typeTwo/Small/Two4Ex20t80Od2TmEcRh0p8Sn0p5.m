function [npDim,pDim_values,nPts_values,nEgs_eachpDim,tmaxVal_eachpDim,nDcol_values,infoCol_values,targetfbest_eachpDim,...
         ...
         =Two4Ex20t80Od2TmEcRh0p8Sn0p5
% Small dim. group ,Od, pDim=[20  40 60 80]
% snr=0.5, rho=0.8, Constant Correlation, trueb type =1

npDim=4   % number of different pDim sets to run
pDim_values=[20  40  60  80]      % values of pDim to run
nPts_values=[100 200 300 400]      % nPts corresponding to each pDim
...
nDcol_values=[0  0  0  0]    % no need to change
infoCol_values=pDim_values    % how many correlated cols for each pDim run
giveCorrParaManually_option=[1 1 1 1]     % manually or randomly assigned for each pDim run
corrflag_option=2*[1  1  1  1]    % constant or exponential corr. for each pDim run
rho_values=[0.8 0.8  0.8  0.8]     % value of rho para for each pDim run
snr_values=0.5*[1  1  1  1]     % value of snr para for each pDim run

% trueb para.
n_non_zeros_trueb_values=[10 10 10 10]     % no. of non-zeros in trueb we want for each pDim run
trueb_type_option=2*[1  1  1  1]     % 0 or 1 for each pDim run
...
nEgs_eachpDim=[1  1  1  1] % nEgs for each run of pDim
