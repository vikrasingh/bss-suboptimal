function [npDim,pDim_values,nPts_values,nEgs_eachpDim,tmaxVal_eachpDim,nDcol_values,infoCol_values,targetfbest_eachpDim,...
               ...
               chooseEgsToRun_eachpDim,egParafile,toUseRealDataFile]=LeukemiaData1ExUdTm1t10
% VS 11/20/22
% Special example input file to run real Steam data set

npDim=1    % number of different pDim sets to run
pDim_values=[3571 ]      % values of pDim to run: 
nPts_values=[72 ]      % nPts corresponding to each pDim:   -------------------- 10x, (1/5)x.

% set 3 tmax para.
tmaxVal_eachpDim=[1:10]   %---------------------------20%, 80%, other 1-2 small about 5 (up to 10%)
                 
targetfbest_eachpDim=zeros(1,10)
toUseRealDataFile={'LeukemiaData'}                     
