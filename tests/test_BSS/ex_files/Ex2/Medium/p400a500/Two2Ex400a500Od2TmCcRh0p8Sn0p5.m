function [npDim,pDim_values,nPts_values,nEgs_eachpDim,tmaxVal_eachpDim,nDcol_values,infoCol_values,targetfbest_eachpDim,...
         ...
         =Two2Ex400a500Od2TmCcRh0p8Sn0p5
...
pDim_values=[400  500]      % values of pDim to run
nPts_values=[2000 2000]      % nPts corresponding to each pDim
...
snr_values=0.5*[1  1]     % value of snr para for each pDim run

% trueb para.
n_non_zeros_trueb_values=[10 10]     % no. of non-zeros in trueb we want for each pDim run
trueb_type_option=2*[1  1]     % 0 or 1 for each pDim run
