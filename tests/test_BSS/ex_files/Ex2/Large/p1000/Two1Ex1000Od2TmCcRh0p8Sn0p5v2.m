function [npDim,pDim_values,nPts_values,nEgs_eachpDim,tmaxVal_eachpDim,nDcol_values,infoCol_values,targetfbest_eachpDim,...
         ...
         =Two1Ex1000Od2TmCcRh0p8Sn0p5v2
% Od, 
% snr=0.5, rho=0.8, Constant Correlation, trueb type =1

npDim=1   % number of different pDim sets to run
pDim_values=[1000]      % values of pDim to run
nPts_values=[4000]      % nPts corresponding to each pDim

% set tmax para.
tmaxVal_eachpDim=[5  10] % each row corresponds to the tmax values of given pDim run     
...
snr_values=0.5*[1 ]     % value of snr para for each pDim run

% trueb para.
n_non_zeros_trueb_values=[5 ]     % no. of non-zeros in trueb we want for each pDim run
trueb_type_option=2*[1 ]     % 0 or 1 for each pDim run
