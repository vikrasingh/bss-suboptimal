...
if infoCol==1  % 24April 2026, include interaction terms X1X2 X1X3 X2X3 not including Xi^2 though
   p2=0.5*(-1+sqrt(1+8*pDim));  
   if mod(p2,1)~=0 % if p2 is not an integer
      error('pDim is not consistent with the number of interactions to be added') 
   end
   nInter=0.5*p2*(p2-1);nMain=pDim-nInter; % X=[(main effect terms)  (inter. terms)]
   rightBlock=zeros(nPts,nInter);
   Xcenter=normalize(X(:,[1:nMain]),'center'); % make mean of columns 0
   ctr=1;
   for i=1:nMain
       for j=(i+1):nMain
          rightBlock(:,ctr)=Xcenter(:,i).*Xcenter(:,j);
          ctr=ctr+1;
       end
   end
   X=[Xcenter rightBlock]; % Xcenter contains the main terms X1, X2, X3, 
                           % rightBlock contains the inteactions X1X2,
                           % X1X3, X2X3
elseif infoCol>0
   if pDim~=infoCol 
