function [rParaOut,X,xtilde,fxtilde,x,fx]=sfs1(p,A,d,c,k,xRelaxedOpt,ixstar,diagInv,targetfbest ,IotherPara,IstopCondPara, iPara,rPara,textfileName,todebug)
% VS:05/21/24
...
Xc=setdiff(1:p,X);  % possible set of variables to be added from
    while stop==0
        ...
        [fXminusi,istar]=leastSigniX(fX,X,p,k, A,d,c,diagInv, iPara,rPara,whichalgo,isSoftStop,isTF,targetfbest);
        % i is index to drop,  fXminusi=f(X - istar)

        % maximize the reduction by adding one variable to Xminusi
        Xminusi=setdiff(X,istar);
        [~,fXplusj,jstar]=mostSigniXc(fXminusi,Xminusi,Xc,p,k, A,d,c,diagInv,  iPara,rPara,whichalgo,isSoftStop, isTF,targetfbest);
...
%% mostSigniXc
function [xout,fstar,istar]=mostSigniXc(fX,X,Xc,p,ki, A,d,c,diagInv,  iPara,rPara,whichalgo,isSoftStop, isTF,targetfbest)
...
% ki=no. of entries in X or predictors already in the model
    ...
    for i=1:(p-ki)
       ...
       [~,xopt,fXplus1]=quad_min_algo_pool(ki,A(idx_kiplus1,idx_kiplus1),d(idx_kiplus1),c,[],[],zeros(ki,1),Sstruct,iPara,rPara,whichalgo,isSoftStop,isTF,targetfbest);
       S=fX-fXplus1;
       if pS<S
...
%% leastSigniX
function [fstar,istar]=leastSigniX(fX,X,p,ki, A,d,c,diagInv,  iPara,rPara,whichalgo,isSoftStop,isTF,targetfbest) 
