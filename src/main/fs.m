function [stopflag,xhat,fhat,X]=fs(p,k,A,d,c,diagInv,iPara,rPara,IotherPara,IstopCondPara,targetfbest)
% VS:19May24
...
Xc=1:p;
ki=1;
[xhat,fXarray(ki),istar]=mostSigniXc(c,X,Xc,p,0, A,d,c,diagInv,  iPara,rPara,whichalgo,isSoftStop,isTF,targetfbest);
...
if k==1
   xout=zeros(p,1);xout(istar)=xhat;xhat=xout; fhat=fXarray; return; 
    ...
    while ki<k 
       [xhat,fXarray(ki+1),istar]=mostSigniXc(fXarray(ki),X,Xc,p,ki, A,d,c,diagInv,  iPara,rPara,whichalgo,isSoftStop,isTF,targetfbest);
       Xc=setdiff(Xc,istar); 
